...
class SGL_1(GeneralGraphRecommender):
    ...
    def __init__(self, config, dataset):
        super(SGL_1, self).__init__(config, dataset)
    ...
    def forward(self, graph=None):
        ...
        if graph is None:  # for the original graph
            for _ in range(self.n_layers):
                ...
                embeddings_list.append( arr[_] * all_embeddings)
            ...
            #         all_embeddings = self.gcn_conv(all_embeddings, graph_edge_index, graph_edge_weight)
            #         # embeddings_list.append(all_embeddings)
            #         embeddings_list.append( arr[i] * all_embeddings)
            
            for i, (graph_edge_index, graph_edge_weight) in enumerate(graph):
                all_embeddings = self.gcn_conv(all_embeddings, graph_edge_index, graph_edge_weight)
                embeddings_list.append(arr[i] * all_embeddings)
