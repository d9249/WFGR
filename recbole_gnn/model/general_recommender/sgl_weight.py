...
class SGL_Weight(GeneralGraphRecommender):
    ...
    def forward(self, graph=None):
        ...
        if graph is None:  # for the original graph
            ...
            for index, (graph_edge_index, graph_edge_weight) in enumerate(graph):
                all_embeddings = self.gcn_conv(all_embeddings, graph_edge_index, graph_edge_weight)
                embeddings_list.append(arr[index] * all_embeddings)
